...
export default function QueryConsole() {
  ...
  const sampleQueries = [
    ...
    { name: 'Patient by Gender', query: 'SELECT gender, COUNT(*) FROM patients GROUP BY gender' },
    { name: 'Recent Patients', query: 'SELECT * FROM patients ORDER BY created_at DESC LIMIT 5' },
    { name: 'Patient by ID', query: 'SELECT * FROM patients WHERE id = $1', params: '1' }
  ];
    ...
    <div className="bg-white shadow-md rounded-lg overflow-hidden">
        ...
        {showHelp && (
          <div className="mb-4 p-3 bg-lime-50 rounded-md text-sm">
            ...
            <ul className="list-disc pl-5 space-y-1 text-blue-900">
              ...
              <li>Press <kbd className="bg-gray-200 px-1 py-0.5 rounded">Ctrl+Enter</kbd> or <kbd className="bg-gray-200 px-1 py-0.5 rounded">⌘+Enter</kbd> to execute the query</li>
              ...
              <div className="border border-gray-300 rounded-md max-h-[192px] overflow-y-auto">
                {queryHistory.map((item, index) => (
                  ...
                  >
                    ...
                    {item.params && (
                      <div className="text-gray-500 truncate">Params: {item.params}</div>
                    )}
                  </div>
                ))}
              </div>
      ...
      {error ? (
        <div className="p-4 bg-red-50 text-red-600 border-t border-red-200">
          ...
          <pre className="text-sm overflow-x-auto p-2 bg-red-100 rounded">{error}</pre>
        </div>
      ) : (
        <div className="border-t border-gray-200">
          <div className="p-4 bg-gray-50 text-gray-700 text-sm font-medium flex justify-between items-center">
            <span>Query Results {results?.rowCount !== undefined && `(${results.rowCount} rows)`}</span>
